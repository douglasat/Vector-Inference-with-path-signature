...
def compute_RLapproximation(viaPoints_selected):
    bnd = [(-scenario.vmax, scenario.vmax), (-scenario.vmax, scenario.vmax), (0.2, 5)]  # bounds
    ineq_cons1 = {'type': 'ineq', 'fun': lambda z: velMax(z, viaPoints_selected, scenario.vmax, u, i)}  # constrains

    u = []
    for k in range(len(viaPoints_selected) - 1):
        u.append([0, 0, 5])
    ...
    while (np.linalg.norm(prevFun - nowFun) >= 1e-3) and count_min < 5:  # find the velocity and td for each via points
        prevFun = nowFun
        for i in range(len(viaPoints_selected) - 1):
    ...
    qx, qy = rolloutViapoints(u, viaPoints_selected)  # compute the full path trajectory with all viapoints terms
...
def getEstimationPath(init, goals, vmax):  # compute an estimation from init to each goal in the set
    ...
    for gk, g in enumerate(goals):
        if str(init) != str(g):
            ...
            viaPoints = optimalTrajectory.geometric_plan(init, g, scenario, 'multi')[0]  # compute the via points


            # Create a ProcessPoolExecutor
            with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
                # Submit tasks to the executor
                futures = [executor.submit(compute_RLapproximation, via) for via in viaPoints[0:10]]
            ...
            if str(state_init) != str(g):
                ...
                for x, y in zip(x_approximeted[str(g)], y_approximeted[str(g)]):
                    ...
                    for a in range(len(Ox)):
                        if len(x) - 1 >= sampled_obser[a]:  # compute the sum of error
                ...
                #p = sum(p_group)/len(p_group) #average among the solution set
                p = max(p_group)
                prob[g_index] = p
