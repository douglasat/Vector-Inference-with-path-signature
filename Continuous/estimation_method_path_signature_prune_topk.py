...
def apply_shift(signal, shift):
    """
    Apply a generic 2D shift to a signal.
    
    Args:
        signal (np.ndarray): The 2D signal array to shift.
        shift (tuple): A tuple (row_shift, col_shift) specifying the shift.
        
    Returns:
        np.ndarray: The shifted signal with wrapped values set to zero.
    """
    row_shift, col_shift = shift
    
    # Shift the array
    shifted_signal = np.roll(signal, shift=row_shift, axis=0)  # Row shift
    shifted_signal = np.roll(shifted_signal, shift=col_shift, axis=1)  # Column shift
    
    # Zero out the wrapped-around values
    if row_shift > 0:  # Downward row shift
        shifted_signal[:row_shift, :] = 0
    elif row_shift < 0:  # Upward row shift
        shifted_signal[row_shift:, :] = 0

    if col_shift > 0:  # Rightward column shift
        shifted_signal[:, :col_shift] = 0
    elif col_shift < 0:  # Leftward column shift
        shifted_signal[:, col_shift:] = 0
...
def corr(signal1, signal2):
    ...
    # Correlation result is larger, so adjust for offsets
    shift = (max_index[0] - (signal2.shape[0] - 1), 
            max_index[1] - (signal2.shape[1] - 1))

    shifted_signal = np.array(apply_shift(signal2, shift))
    return np.array(sig.get_signature(shifted_signal.T)) 
...
def getEstimationPath(init, goals, trees, roots):  # compute an estimation from init to each goal in the set
    x = {}
    y = {}
    theta = {}
    dotx = {}
...
def vector_inference_multi(initial, goal):
    ...
    x_approximated, y_approximated = getEstimationPath(state_init, scenario.goalPoints, trees, roots)
    ...
    for obs in sampled_obser:
        ...
        for g, g_index in zip(scenario.goalPoints, range(len(scenario.goalPoints))):
            nodes_level = trees[str(g)].get_node_by_level(obs)
            if str(g) in goal_hip and len(nodes_level) > 0:
                p_group = []
                ...
                for n, dist in zip(nodes_level, node_dist):
                    error = np.linalg.norm(sig_path_observations - n.identifier) # inference process
                    p_group.append(error)   
