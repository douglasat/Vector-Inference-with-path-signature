...
def select_goal_group(vector, error):
    ...
    k_range = range(2, 7)  # Start from 2 for silhouette scores
        ...
        for g, g_index in zip(scenario.goalPoints, range(len(scenario.goalPoints))):
            if str(g) in goal_hip:
                ...
                for x, y in zip(x_approximated[str(g)], y_approximated[str(g)]):
                    ...
                    fechet_group.append(distance_dtw) 

                    # Observations path signatures
                    gx = np.full(obs + 1, g[0])
                    gy = np.full(obs + 1, g[1])
                    path = np.array([O_Optimal[0, 0:obs+1], O_Optimal[1, 0:obs+1], gx, gy])
                    ...
                    if len(signatures_by_level) - 1 >= obs:
                        ...
                        error = np.linalg.norm(sig_path_observations - np.array(signatures_by_level[-1])) # inference process
                        sig_group.append(error)

                    print([error, distance_dtw])

                prob[g_index] = np.min(sig_group) 
                prob_dw[g_index] = np.min(fechet_group)
...
if __name__ == "__main__":
    ...
    for points in groupPoints:
        ...
        # obs = vector_inference_multi(0, 6)[0]
        # print(obs)
